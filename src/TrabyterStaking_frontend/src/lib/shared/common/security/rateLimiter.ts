...
export class RateLimiter {
    ...
    public checkRateLimit(sourceIdentifier: AppIdentifier): boolean {
        ...
        if (messagesInLastSecond > this.config.maxMessagesPerSecond) {
            console.error(
                `❌ Rate limit exceeded: ${sourceIdentifier} sent ${messagesInLastSecond} messages in 1 second (max: ${this.config.maxMessagesPerSecond})`,
            );
        ...
        if (messagesInLastMinute > this.config.maxMessagesPerMinute) {
            console.error(
                `❌ Rate limit exceeded: ${sourceIdentifier} sent ${messagesInLastMinute} messages in 1 minute (max: ${this.config.maxMessagesPerMinute})`,
            );
            this.blockSource(sourceIdentifier);
            return false;
        }
    ...
    private blockSource(sourceIdentifier: AppIdentifier): void {
        ...
        if (record) {
            ...
            console.warn(
                `🚫 ${sourceIdentifier} blocked for ${blockDurationSeconds} seconds due to rate limit violation`,
            );
        }
    }
     ...
     */
    public getStatistics(sourceIdentifier: AppIdentifier): {
